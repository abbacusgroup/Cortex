...
def _obj(
    ...
    return store.create(
        obj_type=obj_type, title=title, content=title, project=project,
    )
...
class TestCausalChain:
    def test_caused_by_includes_both(
        self, store: Store, gq: GraphQueries,
    ):
    ...
    def test_chain_terminates_at_max_depth(
        ...
        for i in range(4):
            store.create_relationship(
                from_id=ids[i], rel_type="causedBy", to_id=ids[i + 1],
            )
    ...
    def test_circular_chain_terminates(
        ...
        store.create_relationship(
            from_id=id_b, rel_type="causedBy", to_id=id_c,
        )
        store.create_relationship(
            from_id=id_c, rel_type="causedBy", to_id=id_a,
        )
...
class TestEntityNeighborhood:
    ...
    def test_two_hop_connections(
        ...
        eid = store.create_entity(
            name="Docker", entity_type="technology",
        )
...
class TestEvolutionTimeline:
    def test_supersedes_order(
        ...
        # "v2 supersedes v1"
        store.create_relationship(
            from_id=id_new, rel_type="supersedes", to_id=id_old,
        )
    ...
    def test_three_step_chain(
        ...
        store.create_relationship(
            from_id=id_b, rel_type="supersedes", to_id=id_a,
        )
        store.create_relationship(
            from_id=id_c, rel_type="supersedes", to_id=id_b,
        )
...
class TestProjectOverview:
    def test_returns_objects_and_edges(
        ...
        assert any(
            e["from"] == id_a and e["to"] == id_b for e in overview["edges"]
        )
