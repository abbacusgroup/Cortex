...
class TestSymmetry:
    ...
    def test_symmetric_contradicts(self, graph, reasoner):
        ...
        graph.create_relationship(from_id=id_a, rel_type="contradicts", to_id=id_b)
        ...
        outgoing = [
            r for r in rels_b if r["direction"] == "outgoing" and r["rel_type"] == "contradicts"
        ]
...
class TestInverse:
    ...
    def test_caused_by_infers_led_to(self, graph, reasoner):
        id_a, id_b = _create_pair(graph)
        graph.create_relationship(from_id=id_a, rel_type="causedBy", to_id=id_b)
        ...
        rels_b = graph.get_relationships(id_b)
        outgoing_led = [
            r for r in rels_b if r["direction"] == "outgoing" and r["rel_type"] == "ledTo"
        ]
        assert any(r["other_id"] == id_a for r in outgoing_led)

    def test_led_to_infers_caused_by(self, graph, reasoner):
        id_a, id_b = _create_pair(graph)
        graph.create_relationship(from_id=id_a, rel_type="ledTo", to_id=id_b)
        ...
        rels_b = graph.get_relationships(id_b)
        outgoing_caused = [
            r for r in rels_b if r["direction"] == "outgoing" and r["rel_type"] == "causedBy"
        ]
...
class TestTransitivity:
    ...
    def test_one_hop_transitivity(self, graph, reasoner):
        ...
        graph.create_relationship(from_id=id_a, rel_type="supersedes", to_id=id_b)
        graph.create_relationship(from_id=id_b, rel_type="supersedes", to_id=id_c)

        result = reasoner.run()

        assert result["rule_counts"].get("transitive_supersedes_1hop", 0) >= 1
        rels_a = graph.get_relationships(id_a)
        supersedes_targets = [
            r["other_id"]
            for r in rels_a
            if r["direction"] == "outgoing" and r["rel_type"] == "supersedes"
        ]
    ...
    def test_ten_deep_chain(self, graph, reasoner):
        ...
        ids = []
        for i in range(11):
            obj_id = graph.create_object(obj_type="decision", title=f"Node-{i}")
            ids.append(obj_id)

        for i in range(10):
            graph.create_relationship(from_id=ids[i], rel_type="supersedes", to_id=ids[i + 1])
        ...
        for later_id in ids[1:]:
            assert later_id in supersedes_targets, (
                f"Missing transitive supersedes from {ids[0][:8]} to {later_id[:8]}"
            )
    ...
    def test_circular_transitivity_terminates(self, graph, reasoner):
        ...
        graph.create_relationship(from_id=id_a, rel_type="supersedes", to_id=id_b)
        graph.create_relationship(from_id=id_b, rel_type="supersedes", to_id=id_c)
        graph.create_relationship(from_id=id_c, rel_type="supersedes", to_id=id_a)
